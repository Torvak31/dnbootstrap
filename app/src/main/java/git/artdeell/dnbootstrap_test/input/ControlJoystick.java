...
/*public class ControlJoystick extends View implements LayoutTouchConsumer {
    ...
    private void sendKey(int code, boolean pressed) {
        ...
        if (code == -1) {
            GLFW.sendMouseEvent(KeyCodes.SPECIAL_KEY_MOUSE_LEFT, state, 0);
        } else if (code == -2) {
            GLFW.sendMouseEvent(KeyCodes.SPECIAL_KEY_MOUSE_RIGHT, state, 0);
        } else if (code == -3) {
            GLFW.sendMouseEvent(KeyCodes.SPECIAL_KEY_MOUSE_MIDDLE, state, 0);
        } else if (code == -4) {
    ...
    public abstract getVisibilityConfiguration() {
        return null;
    }
}
*/